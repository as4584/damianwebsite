...
for pattern in "${FORBIDDEN_PATTERNS[@]}"; do
  if grep -r "$pattern" ./frontend/app ./frontend/lib --exclude-dir=node_modules --exclude-dir=.next > /dev/null 2>&1; then
...
REQUIRED_ENV_FILES=("frontend/.env.example" "frontend/.env.production")
...
CRITICAL_FILES=(
  ...
  "backend/scripts/wait-for-app.sh"
