...
if [ ! -f "frontend/app/api/health/route.ts" ]; then
  echo "❌ CRITICAL: Health endpoint missing at frontend/app/api/health/route.ts"
  VIOLATIONS=$((VIOLATIONS + 1))
  exit 1
else
...
for pattern in "${FORBIDDEN_IN_SOURCE[@]}"; do
  ...
  if grep -r "$pattern" frontend/app/ frontend/lib/ --exclude-dir=node_modules --exclude="*.test.ts" --exclude="*.test.tsx" --exclude="*middleware*" 2>/dev/null | grep -v ".next" | grep -v "//" | grep -v "/*" | head -3; then
...
if [ ! -x "backend/scripts/wait-for-app.sh" ]; then
  echo "❌ CRITICAL: backend/scripts/wait-for-app.sh is not executable"
  VIOLATIONS=$((VIOLATIONS + 1))
else
